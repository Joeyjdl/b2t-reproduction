...
from fairfaces_pseudo import CustomFairfaces
...
def prepare_celeba(args):
    ...
    valid_indices = list()
    for i in range(4):
        #valid_indices.append( np.where(group == i)[0] [:int(len(np.where(group == i)[0])*args.val_frac)]   ) # original
        valid_indices.append( np.where(group == i)[0] [:int(len(np.where(group == i)[0])*1)]   )
...
def prepare_fairfaces(args):
    ...
    celeba_train = CustomFairfaces(
                        ...
                        transform=transform_train, 
                        pseudo_bias=args.pseudo_bias)
    celeba_valid = CustomFairfaces(
                        root=args.data_root,
                        split='val',      # 'valid'
                        target_attr=args.target_attr, 
                        bias_attr=args.bias_attr,
                        transform=transform_valid)
    celeba_test = CustomFairfaces(
                        root=args.data_root,
                        split='test', 
                        target_attr=args.target_attr, 
                        bias_attr=args.bias_attr,
